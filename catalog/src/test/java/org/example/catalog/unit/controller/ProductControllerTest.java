...
class ProductControllerTest {
  ...
  void getAll_shouldReturnAllProducts() throws Exception {
    ...
    for (int i = 0; i < mockedProducts.size(); i++) {
      ...
      resultActions
          ...
          .andExpect(
              jsonPath(String.format("$[%d].description", i)).value(product.getDescription()))
  ...
  void delete_shouldDeleteProduct() throws Exception {
    mockMvc.perform(delete(PRODUCT_BY_ID_URL, PRODUCT_ID)).andExpect(status().isOk());
  }
  ...
  void update_shouldUpdateProduct() throws Exception {
    ...
    mockMvc
        .perform(put(PRODUCT_URL).contentType(MediaType.APPLICATION_JSON).content(json))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.id").value(mockedProduct.getId()))
        .andExpect(jsonPath("$.name").value(mockedProduct.getName()))
  ...
  void update_invalidRequestBody() throws Exception {
    mockMvc
        .perform(put(PRODUCT_URL).contentType(MediaType.APPLICATION_JSON).content(INVALID_JSON))
        .andExpect(status().isBadRequest());
  }
