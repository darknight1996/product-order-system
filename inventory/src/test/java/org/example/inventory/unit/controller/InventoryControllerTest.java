...
class InventoryControllerTest {
    ...
    void getAll_shouldReturnAllInventories() throws Exception {
        ...
        for (int i = 0; i < inventories.size(); i++) {
            ...
            resultActions.andExpect(jsonPath(String.format("$[%d].id", i)).value(inventory.getId()))
                    .andExpect(jsonPath(String.format("$[%d].productId", i)).value(inventory.getProductId()))
                    .andExpect(jsonPath(String.format("$[%d].productName", i)).value(inventory.getProductName()))
                    .andExpect(jsonPath(String.format("$[%d].productPrice", i)).value(inventory.getProductPrice()))
    ...
    void adjustInventory_shouldAdjustInventory() throws Exception {
        ...
        mockMvc.perform(post(INVENTORY_ADJUST_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isOk());
    }
